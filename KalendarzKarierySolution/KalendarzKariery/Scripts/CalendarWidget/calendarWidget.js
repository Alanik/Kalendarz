    ...
    function calendarWidget($calendar, params) {
        ...
        for (d = 0; d < 42; d++) {
            ...
            if (d < firstDay) {
                ...
                calendar += ('<div class="other-month-cell other-month-day' + currentDay + ' prev-month-cell calendar-cell"  dayNumber="' + (prev_days - firstDay + dayPlusOne) + '"  weekday="' + weekdayModulo + '" data-bind="click:function(){ $root.moveToDetailsPageOnCalendarCellClick($element) }">');
                ...
                if (!(dayPlusOne < firstDay)) {
                    calendar += ('<span class="cell-month-name-span">' + opts.monthNames[prev_m] + '</span>');
                }
                ...
                calendar += ('<div class="other-month-cell other-month-day' + dayCounter + ' next-month-cell calendar-cell" dayNumber="' + dayCounter + '"  weekday="' + weekdayModulo + '" data-bind="click:function(){ $root.moveToDetailsPageOnCalendarCellClick($element) }">');
                calendar += ('<div class="calendar-cell-placeholder"><div class="cell-span-container"><span class="day">' + dayCounter + '</span>');
                if (dayPlusOne == 1) {
                    calendar += ('<span class="cell-month-name-span">' + opts.monthNames[next_m] + '</span>');
                }
                calendar += (' <div class="addNewEvent-cellIcon dark-icon" data-bind="click:function(data, e){ $root.showAddPrivateCalendarEventPopupOnClick($element, data, e)}">+</div></div></div></div>');
                ...
                calendar += ('<div class="current-month-cell  calendar-cell day' + currentDay + '" dayNumber="' + currentDay + '" weekday="' + weekdayModulo + '" data-bind="click:function(){ $root.moveToDetailsPageOnCalendarCellClick($element) }">');
                calendar += ('<div class="calendar-cell-placeholder"><div class="cell-span-container"><span class="day">' + currentDay + '</span>');
                if (currentDay == 1) {
                    calendar += ('<span class="cell-month-name-span">' + opts.monthNames[opts.month] + '</span>');
                }
