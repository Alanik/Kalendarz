function AppViewModel(date, userName, spinner) {
    ...
    self.showSelectedEvents = function (selectedEventsProp, oldOrUpcoming, eventKindValue) {
        ...
        if (self.currentPage == 2) {
            ...
            if ((oldOrUpcoming == 'old' || oldOrUpcoming == 'all')) {
                if (self.detailsPageJournalMenu.menuItems.myCalendar.showOld()) {
                    show(eventKindValue, 'old', selectedEventsProp.old, self.myEventTree);
                } else {
            ...
            if ((oldOrUpcoming == 'upcoming' || oldOrUpcoming == 'all')) {
                if (self.detailsPageJournalMenu.menuItems.myCalendar.showUpcoming()) {
                    show(eventKindValue, 'upcoming', selectedEventsProp.upcoming, self.myEventTree);
                } else {
            ...
            if ((oldOrUpcoming == 'old' || oldOrUpcoming == 'all')) {
                if (self.lobbyPagePublicEventListMenu.menuItems.publicEvents.showOld()) {
                    show(eventKindValue, 'old', selectedEventsProp.old, self.publicEventTree);
                } else {
                    selectedEventsProp.old([]);
                }
            }
            ...
            if ((oldOrUpcoming == 'upcoming' || oldOrUpcoming == 'all')) {
                if (self.lobbyPagePublicEventListMenu.menuItems.publicEvents.showUpcoming()) {
                    show(eventKindValue, 'upcoming', selectedEventsProp.upcoming, self.publicEventTree);
                } else {
                    selectedEventsProp.upcoming([]);
                }
            }
        ...
        function show(eventKindValue, oldOrUpcoming, selectedEventsOldOrUpcoming, eventTree) {
            var arr, shownEvents, combinedArray;

            if ($.isArray(eventKindValue)) {
                arr = self.EVENT_MANAGER.getFilteredEventsFromEventTree(eventTree, ["kind", "value"], eventKindValue, oldOrUpcoming);
            } else {
                arr = self.EVENT_MANAGER.getFilteredEventsFromEventTree(eventTree, ["kind", "value"], [eventKindValue], oldOrUpcoming);
            }
    ...
    self.moveToDetailsDayOnEventCalendarIconClick = function (id, year, month, day) {
        ...
        var events = self.EVENT_MANAGER.getEventsForGivenDay(year, month, day)
        self.detailsPageDayEvents(events);
